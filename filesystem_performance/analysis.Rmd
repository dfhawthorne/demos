...
```{bash download_and_compile_code, eval=FALSE}
...
```{bash generate_test_results, eval=FALSE}
...
```{r summary_of_raw_data, data=raw_data}
summary(raw_data)
```
...
```{r wall_clock_time_vs_user_time, data=raw_data}
plot(
...
```{r wall_clock_time_vs_system_time, data=raw_data}
plot(
...
```{r effect_of_blksize_on_system_time, fig.cap='Effect of Block Size  (kb) on System Time (s)'}
boxplot(
...
```{r effect_of_block_size_on_log_system_time, fig.cap='Effect of Block Size on System Time - Logarithmic scale'}
boxplot(
  Sys_Time ~ Block_Size_K,
...
```{r effect_of_osync_on_system_time}
boxplot(
  Sys_Time ~ Sync,
  data=raw_data[raw_data$Sys_Time > 0.0, ],
...
```{r effect_of_output_file_size_on_system_time}
boxplot(
...
```{r effect_of_blocks_written_on_system_time}
...
```{r effect_on_sys_time_from_blks_written_no_osync}
boxplot(
  Sys_Time ~ Blocks_Written,
...
```{r effect_on_sys_time_from_blks_written_with_osync}
boxplot(
  Sys_Time ~ Blocks_Written,
