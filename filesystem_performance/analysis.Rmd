...
* **Buffer Size** refers to the size of memory set aside for I/O calls (such as write). In the test program, the buffer is sized to be one (1) block in size.
...
I will keep them at this stage and rely on models that use linear combinations of the treatment variables.
...
First, I will want to see what the effect on *Sys_Time* from different levels of *Blocks_Written*  when the *O_SYNC* flag is set:
```{r}
boxplot(
  Sys_Time ~ Blocks_Written,
...
Second, I will want to see what the effect on *Sys_Time* from different levels of *Blocks_Written* when the *O_SYNC* flag is set:
```{r}
boxplot(
  Sys_Time ~ Blocks_Written,
